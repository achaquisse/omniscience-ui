...
export const DateRangePicker = ({
  ...
  const PresetButton = ({ preset, label, isSelected }) => (
    <Button
      className={cn('w-full justify-start text-left font-normal h-9', isSelected && 'bg-accent')}
      variant="ghost"
      size="sm"
      onClick={() => {
        setPreset(preset)
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <Button size={'lg'} variant="outline" className={cn('w-full justify-start text-left font-normal')}>
          <div className="flex w-full items-center gap-2 overflow-auto">
            <div className="flex items-center">
              <div>
                {`${formatDate(range.from, locale)}${range.to != null && range.to.getTime() !== range.from.getTime() ? ' - ' + formatDate(range.to, locale) : ''}`}
              </div>
            </div>
            {rangeCompare != null && (
              <div className="flex items-center">
      ...
      <PopoverContent align={align} className="w-auto p-0">
        <div className="flex">
              ...
              <Calendar
                ...
                onSelect={(value) => {
                  if (isSelectingNewRange) {
                    const clickedDate = value?.from || value?.to || value
                    setRange({ 
                      from: clickedDate, 
                      to: undefined
                    })
                ...
                selected={isSelectingNewRange ? undefined : range}
          ...
          {!isSmallScreen && (
            <div className="flex flex-col gap-1 border-l px-2 py-3">
              {PRESETS.map((preset) => (
                <PresetButton
                  key={preset.name}
                  preset={preset.name}
        ...
        <div className="flex justify-end gap-2 border-t px-3 py-2">
          ...
          <Button
            onClick={() => {
              ...
              if (
                !areRangesEqual(finalRange, openedRangeRef.current) ||
                !areRangesEqual(rangeCompare, openedRangeCompareRef.current)
              ) {
                onUpdate?.({ range: finalRange, rangeCompare })
              }
            }}
          >
            Update
