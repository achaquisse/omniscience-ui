...
export default function AttendanceClasses() {
  ...
  useEffect(() => {
    const checkTodayAttendance = async () => {
      if (!accessToken || classes.length === 0) return

      const attendanceChecks = classes.map(async (cls) => {
        try {
  ...
  }, [accessToken, classes, today])

  return (
    <div className="space-y-4 sm:space-y-6">
      <h1 className="text-2xl sm:text-3xl font-bold">Select a Class</h1>
