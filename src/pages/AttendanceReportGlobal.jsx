...
export default function AttendanceReportGlobal() {
  ...
  useEffect(() => {
    const initializeDates = async () => {
      ...
      try {
        ...
        setCourseName(studentClass.Course?.Name || '')
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">Attendance Report of {className || 'Class'} - {courseName || 'Course'}</h1>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span
                className="text-2xl sm:text-3xl font-bold">{formatPercentage(report.overallSummary?.percentage || 0)}</span>
            </div>
          </CardContent>
        </Card>
              ...
              {topStudents.map((student, index) => (
                <div key={student.studentId} className="flex items-center justify-between p-2 sm:p-3 border rounded-lg">
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    <div
                      className="flex items-center justify-center size-7 sm:size-8 rounded-full bg-primary/10 text-primary font-semibold text-xs sm:text-sm flex-shrink-0">
                      {index + 1}
                    </div>
      ...
      {report.studentSummaries?.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs sm:text-sm">Complete attendance summary for all
              ...
              <table className="w-full text-xs sm:text-sm">
                ...
                {report.studentSummaries.map((student) => (
                  <tr key={student.studentId} className="border-b hover:bg-gray-50">
                    ...
                    <td
                      className="text-center py-2 sm:py-3 px-1 sm:px-2 text-green-600 font-medium">{student.presentCount}</td>
                    <td
                      className="text-center py-2 sm:py-3 px-1 sm:px-2 text-red-600 font-medium">{student.absentCount}</td>
                    <td
                      className="text-center py-2 sm:py-3 px-1 sm:px-2 text-yellow-600 font-medium">{student.lateCount}</td>
                    <td
                      className="text-center py-2 sm:py-3 px-1 sm:px-2 text-blue-600 font-medium">{student.excusedCount}</td>
                    <td
                      className="text-center py-2 sm:py-3 px-1 sm:px-2 font-semibold">{formatPercentage(student.percentage)}</td>
                  </tr>
                ))}
                </tbody>
              </table>
