...
export default function AttendanceReportIndividual() {
  ...
  useEffect(() => {
    const initializeDates = async () => {
      if (!accessToken || !classId || !studentId || datesInitialized) return
    ...
    initializeDates()
  }, [accessToken, classId, studentId, datesInitialized, today])
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <h1 className="text-3xl font-bold">Attendance Report of {studentName || 'Student'}</h1>
        </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-3 sm:gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">Attendance Report of {studentName || 'Student'}</h1>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span
                className="text-2xl sm:text-3xl font-bold">{formatPercentage(report.summary?.percentage || 0)}</span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {report.records.map((record, index) => (
                <div key={index}
                     className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3 p-2 sm:p-3 border rounded-lg">
                  ...
                  {record.remarks && (
                    <span
                      className="text-xs text-muted-foreground italic line-clamp-2 sm:line-clamp-1">{record.remarks}</span>
                  )}
                </div>
              ))}
            </div>
